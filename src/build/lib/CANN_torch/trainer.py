...
from CANN_torch.models.CNN import *
# from models.CNN import StrainEnergyCANN, StrainEnergyCANN_C, StrainEnergyCANN_polinomial3
from CANN_torch.utils.dataload import ExcelDataset
# from CANN_torch.models import CNN
# from CANN_torch.utils import dataload
...
class Trainer:
    def __init__(self,
                 ...
                 SingleInvNet: Optional[nn.Module] = None,
                 terms_count: int = 6,
        ...
        if SingleInvNet is None:
            if terms_count == 6:
                self.SingleInvNet = SingleInvNet_6()

            elif terms_count == 4:
                self.SingleInvNet = SingleInvNet_4()
        else:
            self.SingleInvNet = SingleInvNet
            self.terms_count = SingleInvNet.__name__[-1]
    ...
    def train(self,
        ...
        if train_mode == "one_mode":
            ...
            # psi_model = PsiModel(self.SingleInvNet, self.device).to(self.device)
            combined_model = CANN_CombinedModel(self.SingleInvNet).to(self.device)
            self.model = combined_model
            optimizer = optim.Adam(self.model.parameters(), lr=self.learning_rate)
