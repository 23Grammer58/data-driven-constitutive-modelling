...
def load_and_extract(file_path, experiment_type):
    ...
    return df[['lambda_x', 'lambda_y', 'stress_x_mpa', 'stress_y_mpa', 'cycle_number']]
...
def preproc_data(experiment_protocols: list = None):
    ...
    data_frames = [load_and_extract(file, file[:-4]) for file in experiments_path] # поменять для друго даты
